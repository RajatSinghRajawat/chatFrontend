...
export default function LoginPage() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (!response.ok) {
        setSnackbar({ open: true, message: resData.msg || 'Login failed', severity: 'error' });
        setError('api', { message: resData.msg || 'Login failed' });
      } else {
        setSnackbar({ open: true, message: 'Login successful!', severity: 'success' });
        localStorage.setItem('token', resData.token);
        localStorage.setItem('userdatachat', JSON.stringify(resData.user));
        setTimeout(() => navigate('/chat'), 1000);
      }
    } catch (err) {
      setSnackbar({ open: true, message: 'Login failed', severity: 'error' });
      setError('api', { message: 'Login failed' });
    } finally {
      setLoading(false);
    }
  };
        ...
        <Grid item xs={12} md={4} sx={{
          ...
          <Box sx={{ width: '100%', maxWidth: 400, textAlign: { xs: 'center', md: 'left' } }}>
            ...
            <Typography variant="body1" sx={{ mb: 3, color: 'rgba(255,255,255,0.7)', fontSize: 16, fontFamily: 'Poppins, sans-serif' }}>
              ...
              you can <Link href="/register" sx={{ color: '#4F8CFF', fontWeight: 600, fontFamily: 'Poppins, sans-serif' }}>Register here!</Link>
            </Typography>
          </Box>
        </Grid>
        {/* Center Illustration */}
                ...
                <Stack spacing={2}>
                  ...
                  <Box>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 0.5 }}>
                      <Link href="#" sx={{ color: '#aaa', fontSize: 14, textDecoration: 'none', '&:hover': { color: '#4F8CFF' }, fontFamily: 'Poppins, sans-serif' }}>
                        Recover Password ?
                      </Link>
                    </Box>
                  </Box>
